...
# Define named tuple for track geometry
TrackGeometry = namedtuple('TrackGeometry', [
...
def build_track_from_example_lap(example_lap, track_width=200):
    ...
    return TrackGeometry(
        plot_x_ref=plot_x_ref, plot_y_ref=plot_y_ref,
...
class F1ReplayWindow(arcade.Window):
    def __init__(self, frames, track_statuses, example_lap, drivers, title,
                ...
                for filename in os.listdir(tyres_folder):
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        try:
                            ...
                            print(f"Warning: Failed to load tyre texture {filename}: {e}")
            except Exception as e:
                print(f"Warning: Failed to read tyres folder: {e}")

        # Build track geometry (Raw World Coordinates)
        try:
            track_geom = build_track_from_example_lap(example_lap)
            self.plot_x_ref = track_geom.plot_x_ref
            self.plot_y_ref = track_geom.plot_y_ref
            self.x_inner = track_geom.x_inner
            self.y_inner = track_geom.y_inner
            self.x_outer = track_geom.x_outer
            self.y_outer = track_geom.y_outer
            self.x_min = track_geom.x_min
            self.x_max = track_geom.x_max
            self.y_min = track_geom.y_min
            self.y_max = track_geom.y_max
        except Exception as e:
            print(f"Error building track geometry: {e}")
            raise
        ...
        if os.path.exists(bg_path):
            try:
                ...
                print(f"Warning: Failed to load background texture: {e}")
    ...
    def on_draw(self):
        ...
        try:
            ...
            print(f"Warning: Error processing track status: {e}")

        # Map track status -> colour (R,G,B) - cached colors
        STATUS_COLORS = {
        ...
        # Map numeric statuses to color keys for efficient lookup
        status_color_map = {
            "2": "YELLOW",
            "4": "SC",
            "5": "RED",
        ...
        track_color = STATUS_COLORS.get(status_color_map.get(current_track_status, "GREEN"), (150, 150, 150))
 
        if self.screen_inner_points:
            arcade.draw_line_strip(self.screen_inner_points, track_color, 4)
        if self.screen_outer_points:
        ...
        try:
            ...
            print(f"Warning: Error drawing cars: {e}")
        ...
        try:
            ...
            print(f"Warning: Error calculating driver progress: {e}")
        ...
        for i, (code, color, pos, progress_m) in enumerate(driver_list):
            ...
            try:
                ...
                print(f"Warning: Error drawing tyre icon: {e}")
        ...
        if self.selected_driver and self.selected_driver in frame["drivers"]:
            try:
                ...
                print(f"Warning: Error drawing selected driver info: {e}")
